...
export function renderVideoPlayerDirective(directive: ParsedDirective): string {
  ...
  const hasAssetId = typeof attrs.videoAssetId === 'string' && attrs.videoAssetId.trim().length > 0

  if (!hasSrc && !hasStream && !hasAssetId) {
    return renderInvalidDirective(directive.rawName || 'video-player', 'missing_src_stream_or_asset_id')
  }
