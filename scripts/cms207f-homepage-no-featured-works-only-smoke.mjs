...
if (/\.filter\(\s*\(?\s*entry\s*\)?\s*=>\s*entry\.featured\s*\)/.test(component)) fail('homepage recent feed must not be featured-only')
if (!component.includes('recentEntries')) fail('homepage recent feed must derive from generated home collection recentEntries')
if (!component.includes('useHomeCollections')) fail('homepage recent feed must use CMS-207M-R1 derived home collection source')
