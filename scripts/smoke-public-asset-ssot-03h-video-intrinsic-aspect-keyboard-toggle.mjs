...
assertIncludes(videoPlayer, 'intrinsicWidth', 'VideoPlayer.vue')
assertIncludes(videoPlayer, 'intrinsicHeight', 'VideoPlayer.vue')
assertIncludes(videoPlayer, 'videoWidth', 'VideoPlayer.vue')
assertIncludes(videoPlayer, 'videoHeight', 'VideoPlayer.vue')
assertIncludes(videoPlayer, '--vt-video-aspect-ratio', 'VideoPlayer.vue')
...
assertIncludes(videoPlayer, '@keydown', 'VideoPlayer.vue')
assertIncludes(videoPlayer, '.space.prevent', 'VideoPlayer.vue')
...
assertIncludes(stageBlock, 'aspect-ratio: var(--vt-video-aspect-ratio', '.vt-video-player__stage')
assertIncludes(stageBlock, 'width: min(100%', '.vt-video-player__stage')
assertIncludes(stageBlock, 'margin-inline: auto', '.vt-video-player__stage')

assertIncludes(videoBlock, 'height: 100%', '.vt-video-player__video')
assertIncludes(videoBlock, 'object-fit: contain', '.vt-video-player__video')
assertNotIncludes(videoBlock, 'object-fit: cover', '.vt-video-player__video')

if (failures.length) {
  console.error('FAIL_PUBLIC_ASSET_SSOT_03H_VIDEO_INTRINSIC_ASPECT_KEYBOARD_TOGGLE')
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}

console.log('PASS_PUBLIC_ASSET_SSOT_03H_VIDEO_INTRINSIC_ASPECT_KEYBOARD_TOGGLE')
